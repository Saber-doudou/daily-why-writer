...
**🔒 定位断言必须回读原文校验（09-18 v2.9 新增，硬约束）**

凡在 `issues` / `p2` / `overall_comment` 里写出**具体位置**的断言（「第 N 行有 X」「共 N 处 X」「A段/B段存在 X」），落盘前**必须**逐条回读原文核对，用 Grep 或 Read 数一遍实际出现次数与行号：

- **禁止凭印象给位置或计数**。写「共 2 处」前必须实际数出 2 处；写「第 15 行有 X」前必须 `Read` 该行确认。
- **计数只统计原文现状，不得把自己建议的替换文本算进去**。
- 若某处只是「建议改成什么样」，那是目标文本，**不属于现状描述**，不得计入计数或当作现有问题。

**背景（09-18 实证）**：v1 审校报告称「A段第3行与F段第15行使用双破折号『——』做转折（共2处）」，
实测全文 `——` **仅 1 处**（第 3 行 A 段），第 15 行 F 段用的是逗号。该断言把**自己建议的替换示例文本**
误当成了**原文现状**，属位置与计数双错。此类失实会让后续按位置修文时**改错行**。
依据 EXP-014 可观测性即诚实性：审校报告的定位精度本身就是产出质量的一部分。
