...
v1.1 2026-09-18  新增「加粗词」提取模式（**xxx**，2 至 16 字符）：纯中文行（如铁律常驻行）此前
                  token 提取为 0，「0 丢失」实为「无 token 可查」而非「查过没问题」（09-18 A3 校准
                  实证，靠定向检索人肉补验兜底）；加粗词在记忆文件中即规则名/专名，粒度受控噪音小。
                  验证：ast.parse + 今日 A3 快照正向 + 09-08 历史快照回归 + 缺陷样本反向（LOST exit 1）
...
PATTERNS = [
    ...
    (re.compile(r"\*\*([^*]{2,16})\*\*"), "加粗词"),   # **模型无关性**（v1.1 补纯中文行盲区；16=实测最长「EXP-014 可观测性即诚实性」）
